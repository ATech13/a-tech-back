...
export const InteractiveTerminal: React.FC<InteractiveTerminalProps> = ({
  ...
  // Update welcome message when language changes if only welcome log exists
  useEffect(() => {
    setLogs((prev) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md p-2 sm:p-4 flex min-h-full items-center justify-center animate-fadeIn">
      <div
        className={`bg-[#08140B] border border-[#a3e635] rounded-2xl shadow-2xl flex flex-col transition-all duration-300 my-auto ${
          expanded ? 'w-full h-[95vh]' : 'max-w-2xl w-full h-[85vh] sm:h-[540px]'
        }`}
      >
        {/* Terminal Titlebar */}
        <div className="bg-[#091B0E] px-4 py-3 border-b border-[#a3e635]/30 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-red-500 inline-block" />
            <span className="w-3 h-3 rounded-full bg-yellow-500 inline-block" />
            <span className="w-3 h-3 rounded-full bg-emerald-500 inline-block" />
            <span className="text-xs font-mono text-[#a3e635] ml-2 flex items-center gap-1.5">
              <TerminalIcon className="w-3.5 h-3.5" /> aaron-tech-cli
            </span>
          </div>
        ...
        <form
          ...
          className="bg-[#091B0E] p-3 border-t border-[#a3e635]/20 flex items-center gap-2 rounded-b-2xl"
        >
